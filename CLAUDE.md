...
englisheducation의 디자인·구조 그대로 재사용한다. 음운론/통사론 작업 시 보조 참고 스킬도 함께.
...
| 통사론 예문·트리·OX 작성 보조 | `transformational-grammar-html` (Radford 원서 기준 예문 처리·tree SVG) |
...
> ⚠️ **원칙**: 예문은 반드시 `transformational_grammar.md` 원문 그대로. 규칙 정의도 동일 기준. 나머지 md는 해설 보조·임용 출제 포인트 파악용.
...
- **콘텐츠 작성자(=Claude)가 점검까지 책임진다.** 사용자에게 "미묘한 부분은 확인해주세요" 떠넘기지 않는다.
...
**썸네일**: 신규 글마다 **Canva MCP**로 1장 생성 — `mcp__claude_ai_Canva__generate-design` 사용. 사이즈 1200×630 (Tistory 표준), 인디고 그라디언트 배경, 챕터 제목 + 영역명 + "임용영어학".
