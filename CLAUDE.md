# linguistics — CLAUDE.md (Claude Code)
...
- **Git 워크플로우는 Claude Code 환경 기준으로 처리.** 작업 완료 후 필요한 검증을 먼저 하고, 커밋은 사용자가 요청했거나 작업 단위가 명확할 때 수행한다. 푸시·PR·배포처럼 외부에 보이는 작업은 명시 요청이 있을 때 진행한다.
...
englisheducation의 디자인·구조를 재사용한다. `.claude/skills/`의 정의가 있다면 그대로 호출하고, 없으면 아래 순서로 직접 확인하고 작업한다.

| 상황 | Claude Code 기준 처리 |
|------|----------------|
...
| 이미지 생성 | Canva plugin이 사용 가능하면 Canva, 아니면 Pencil MCP·Gemini 또는 사용자가 지정한 도구 사용 |

Task 도구의 sub-agent는 작업 분할이 의미 있을 때만 사용한다. 자동으로 백그라운드 에이전트를 띄우라는 규칙은 trivial 작업에 강제 적용하지 않는다.
...
- **콘텐츠 작성자(=Claude Code)가 점검까지 책임진다.** 사용자에게 "미묘한 부분은 확인해주세요" 떠넘기지 않는다.
...
- **신규 글 썸네일은 Canva plugin 또는 Pencil MCP·Gemini로 생성**. 기존 글은 그대로 유지
...
**썸네일**: 신규 글마다 가능하면 **Canva plugin**으로 1장 생성. Canva가 현재 세션에서 사용 불가하면 Pencil MCP·Gemini 또는 사용자가 지정한 도구를 사용한다. 사이즈 1200×630 (Tistory 표준), 인디고 그라디언트 배경, 챕터 제목 + 영역명 + "임용영어학".
...
`_study.html` 또는 OX 파일을 **신규 작성하거나 대폭 수정**할 때 아래 절차를 Claude Code가 직접 수행한다. sub-agent는 사용자가 명시적으로 요청한 경우에만 병렬 보조로 사용한다.
